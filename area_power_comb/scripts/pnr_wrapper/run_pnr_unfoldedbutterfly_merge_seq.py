...
if __name__ == "__main__":
  ...
  top_module = ["unfoldedbutterfly_merge_seq"]
  # specify the location from the root path of "area_power"
  directory = "./src/unfoldedbutterfly_merge_seq/"
  # specify the file that should not be added in the synthesis project.
  remove_list = []
  # specify the parameter list
  # parameter_value = [[4, 8, 16, 32, 64, 128, 256, 512],[8, 16, 32, 64, 128, 256, 512, 1024]]
  ...
  for i in range(len(parameter_value[0])):
    ...
    # 4) move report to the report directory
    os.system("mv ./SYNTH/rpt " + "./report/" + top_module[0] + str(parameter_value[1][i]))

    # 5) move report of pnr into /pnr_report
    os.system("mv ./PNR " + "./pnr_report/" + top_module[0] + str(parameter_value[1][i]))
